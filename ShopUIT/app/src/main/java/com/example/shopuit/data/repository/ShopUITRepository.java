...
public class ShopUITRepository {
    ...
    private final MutableLiveData<List<SanPham>> sanphams = new MutableLiveData<>();
    ...
    public MutableLiveData<List<SanPham>> getAllSanPhams(){
        shopUITApiManager.getAllSanPhams(new Callback<List<SanPham>>() {
            @Override
            public void onResponse(Call<List<SanPham>> call, Response<List<SanPham>> response) {
                if (response.isSuccessful()){

                    List<SanPham> body = response.body();
                    sanphams.setValue(body);
                } else{
                    sanphams.postValue(null);
                }
    ...
    public MutableLiveData<List<SanPham>> getSanPhamById(int id){
        shopUITApiManager.getSanPhamById(id , new Callback<List<SanPham>>() {
            @Override
            public void onResponse(Call<List<SanPham>> call, Response<List<SanPham>> response) {
                if (response.isSuccessful()){
                    List<SanPham> body = response.body();
